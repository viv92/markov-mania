# Program implementing policy evaluation from given finite horizon mdp (with explicit end state) using modified backward pass - equivalent to bellmann equation
...
def main(seed):
    ...
    n_states = 3 # K
    n_actions = 2 # A
    seq_len = 100000 # N - this is an upper bound on the horizon. Mostly the horizon will be determined by the terminal state
    df = 1
    random_seed = seed
    ...
    action_probs_true = [[0.8, 0.2], [0.6, 0.4], [0.5, 0.5]] # aka policy; shape = (K, A)
