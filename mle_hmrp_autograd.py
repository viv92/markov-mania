# Program estimating the parameters of an example hidden markov reward process (HMRP) by formulating MLE as a two step algorithm:
...
# p(r_n|z_n) is a gaussian distribution whose mu and sigma are conditioned on the state taken by z_n (thus parameterized by k X 2 parameters)
...
def forward_backward(dataset, n_latent_classes, seq_len, start_probabilities, transition_probabilities, emission_probabilities, reward_mus, reward_sigmas):
    N = seq_len
    S = n_latent_classes
    ...
    curr_obs, curr_reward = dataset[0]
    curr_obs = int(curr_obs)
    for j in range(S):
        alpha[0][j] = start_probabilities[j] * \
                      emission_probabilities[j][curr_obs] * \
                      torch.exp(tdist.Normal(loc=reward_mus[j], scale=reward_sigmas[j]).log_prob(curr_reward))
    # rescale / normalize
    sf = sum(alpha[0])
    alpha[0] /= sf
    ...
    for i in range(1, N):
        curr_obs, curr_reward = dataset[i]
        curr_obs = int(curr_obs)
        for j in range(S):
            for k in range(S):
    ...
    for i in range(N-2, -1, -1):
        next_obs, next_reward = dataset[i+1]
        next_obs = int(next_obs)
        for j in range(S):
            for k in range(S):
                beta[i][j] += beta[i+1][k] * transition_probabilities[j][k] * emission_probabilities[k][next_obs] * \
                              torch.exp(tdist.Normal(loc=reward_mus[k], scale=reward_sigmas[k]).log_prob(next_reward))
...
def mle(dataset, n_latent_classes, n_obs_classes, seq_len, n_iters, n_seq, batch_size, lr):
    ...
    reward_mus = np.random.rand(n_latent_classes)
    reward_sigmas_raw = np.random.rand(n_latent_classes)
    ...
    params = list([start_scores, transition_scores, emission_scores, reward_mus, reward_sigmas_raw])
    ...
    for iter in tqdm(range(n_iters)):
        ...
        for i in range(batch_size):
            ...
            with torch.no_grad():
                responsibilities_marginal, responsibilities_joint = forward_backward(dataset_batch[i], n_latent_classes, seq_len, start_probs, transition_probs, emission_probs, reward_mus, reward_sigmas)
            ...
            for k in range(n_latent_classes):
                ...
                m_step_objective += torch.dot( responsibilities_marginal[:, k], tdist.Normal(loc=reward_mus[k], scale=reward_sigmas[k]).log_prob(dataset_batch[i,:,1]).float() )
    ...
    return start_probs, transition_probs, emission_probs, reward_mus, reward_sigmas
...
def main(seed):
    ...
    reward_mus_true = [2., 7.] # shape (K,)
    reward_sigmas_true = [.5, .5] # shape (K,)

    ## generate dataset

    dataset = np.zeros((n_seq, seq_len, 2)) # each term in a sequence is a tuple (obs, reward)
    ...
    for i in range(n_seq):
        ...
        r_1 = np.random.normal(loc=reward_mus_true[z_1], scale=reward_sigmas_true[z_1])
        dataset[i][0] = x_1, r_1
        ...
        for n in range(1, seq_len):
            ...
            r_n = np.random.normal(loc=reward_mus_true[z_n], scale=reward_sigmas_true[z_n])
            dataset[i][n] = x_n, r_n
            # for next data point
            z_n_minus_1 = z_n
    ...
    starting_latent_class_probs_est, transition_latent_class_probs_est, emission_probs_est, reward_mus_est, reward_sigmas_est = mle(dataset, n_latent_classes, n_obs_classes, seq_len, n_iters, n_seq, batch_size, lr)
