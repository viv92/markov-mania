    ...
    for iter in tqdm(range(n_iters)):
        ...
        for g_step in range(grad_steps):
            ...
            for k in range(n_latent_classes):
                m_step_objective += sumR[0][k] * torch.log(start_probs[k])

            sumR = torch.sum(responsibilities_joint, axis=0)
            sumN = torch.sum(sumR, axis=0)
            ...
            #     #     emission_probs_vector[n] = emission_probs[:, dataset[r][n]]
            #     for k in range(n_latent_classes):
            #         m_step_objective += torch.dot( responsibilities_marginal[r, :, k], tdist.Categorical(emission_probs[k]).log_prob(dataset[r]).float() )
            #         # m_step_objective += torch.dot( responsibilities_marginal[r, :, k], torch.log(emission_probs_vector[:, k]) )

            for k in range(n_latent_classes):
                emission_logprobs = tdist.Categorical(emission_probs[k]).log_prob(dataset).float().T # shape: (N, R)
                m_step_objective += torch.trace(torch.matmul( responsibilities_marginal[:, :, k], emission_logprobs ))
