# Program estimating the parameters of an example hidden markov process (HMP) by formulating MLE as a two step algorithm:
...
def mle(dataset, n_latent_classes, n_obs_classes, seq_len, n_iters, n_seq, batch_size, lr):
    ...
    for iter in tqdm(range(n_iters)):
        ...
        for i in range(batch_size):
            ...
            with torch.no_grad():
                responsibilities_marginal, responsibilities_joint = forward_backward(dataset_batch[i], n_latent_classes, seq_len, start_probs, transition_probs, emission_probs)
            ...
            for k in range(n_latent_classes):
                m_step_objective += torch.dot( responsibilities_marginal[:, k], tdist.Categorical(emission_probs[k]).log_prob(dataset_batch[i]).float() )
...
def main(seed):
    ...
    starting_latent_class_probs_est, transition_latent_class_probs_est, emission_probs_est = mle(dataset, n_latent_classes, n_obs_classes, seq_len, n_iters, n_seq, batch_size, lr)
