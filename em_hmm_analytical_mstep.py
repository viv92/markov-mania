...
def em(dataset, n_latent_classes, n_obs_classes, seq_len, n_iters):
    ...
    for iter in tqdm(range(n_iters)):
        ...
        for k in range(n_latent_classes):
            start_probs[k] = responsibilities_marginal[0][k] / torch.sum(responsibilities_marginal[0])

        # calculate transition probs
        sumN = torch.sum(responsibilities_joint, axis=0) # sumN.shape = (n_latent_classes, n_latent_classes)
        sumKN = torch.sum(sumN, axis=1) # sumKN.shape = (n_latent_classes,)
        for j in range(n_latent_classes):
            for k in range(n_latent_classes):
                transition_probs[j][k] = sumN[j][k] / sumKN[j] # according to bishop eq(13.19)

        # calculate emission_probs (catagorical distribution)
        for k in range(n_latent_classes):
            for i in range(n_obs_classes):
                xi = (dataset == i).float() # indicator vector for obs_class = i
                emission_probs[k][i] = torch.dot(responsibilities_marginal[:, k], xi) / torch.sum(responsibilities_marginal[:, k]) # according to bishop eq(13.23)
